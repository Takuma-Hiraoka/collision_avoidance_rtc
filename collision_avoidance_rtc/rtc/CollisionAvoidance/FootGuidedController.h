...
inline std::ostream &operator<<(std::ostream &os, const std::vector<footguidedcontroller::LinearTrajectory<cnoid::Vector3> > refZmpTraj) {
  for(int i=0;i<refZmpTraj.size();i++){
    os << "refZmpTraj" << i << std::endl;
    os << "start: " << (refZmpTraj[i].getStart()).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", " [", "]")) << std::endl;
    os << "goal: " << (refZmpTraj[i].getGoal()).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", " [", "]")) << std::endl;
    os << "time = " << refZmpTraj[i].getTime() << " [s]" << std::endl;
  }
  return os;
};
